...
interface RemoverUsuarioGrupoModalProps {
  open: boolean;
...
export function RemoverUsuarioGrupoModal({
  open,
  onOpenChange,
  token,
  grupoAcessoId,
  ...
  const removerUsuarioGrupoUseCase = container.resolve(RemoverUsuarioGrupoUseCase);
  ...
  const handleRemover = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await removerUsuarioGrupoUseCase.execute(token, selectedColaborador.usuarioId, grupoAcessoId);
      if (res.sucesso) {
        logUserAction('Permissionamento', 'RemoverUsuarioGrupo', { grupoAcessoId, usuarioId: selectedColaborador.usuarioId }, user);
        toast.success(res.mensagem ?? 'Usuário removido do grupo com sucesso.');
        onSuccess?.();
        setSelectedColaborador(null);
        setSearchTerm('');
      } else {
        toast.error(res.mensagem ?? 'Erro ao remover usuário do grupo.');
      }
    } catch (err: unknown) {
      const msg =
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[420px]">
        <DialogHeader>
          <DialogTitle>Remover usuário do grupo</DialogTitle>
          <DialogDescription>
        ...
        <DialogFooter>
          ...
          <Button variant="destructive" onClick={handleRemover} disabled={submitting || !selectedColaborador}>
            {submitting ? (
              <>
