...
export interface AtribuirRecrutadorCandidatoModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  idCandidatura: string;
  /** Nome do recrutador atual (para pré-selecionar no dropdown). */
  currentRecrutadorNome?: string | null;
  token: string | null;
  onSuccess?: () => void;
}

export function AtribuirRecrutadorCandidatoModal({
  open,
  onOpenChange,
  idCandidatura,
  ...
  token,
  onSuccess,
}: AtribuirRecrutadorCandidatoModalProps) {
  ...
  const atualizarRecrutador = container.resolve(AtualizarRecrutadorResponsavelUseCase);
  ...
  useEffect(() => {
    ...
    listarColaboradoresOrg
      .execute(token, { busca: '', cursor: 0, limite: 999999 })
      .then((res) => {
        if (cancelled) return;
  ...
  const handleSave = async () => {
    if (!selectedCodigo || !token || !idCandidatura) return;
    setSaving(true);
    try {
      const res = await atualizarRecrutador.execute(token, idCandidatura, selectedCodigo);
      if (res?.sucesso !== false) {
        toast.success(res?.mensagem ?? 'Recrutador responsável atualizado com sucesso.');
        onOpenChange(false);
        onSuccess?.();
      } else {
  ...
  const displayValue =
    selectedColaborador?.nm_Profissional ?? selectedColaborador?.cd_Profissional ?? selectedColaborador?.codigoColaboradorInterno ?? '';

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md" aria-labelledby="atribuir-recrutador-candidato-title">
        <DialogHeader>
          <DialogTitle id="atribuir-recrutador-candidato-title">Recrutador(a)</DialogTitle>
          <DialogDescription>Insira o nome do recrutador responsável.</DialogDescription>
        </DialogHeader>
        <div className="space-y-2">
          <Label id="recrutador-candidato-label">Recrutador</Label>
          {loading ? (
            ...
            <Popover open={recrutadorPopoverOpen} onOpenChange={setRecrutadorPopoverOpen} modal>
              <PopoverTrigger asChild>
                <Button
                  id="recrutador-candidato-select"
                  variant="outline"
                  role="combobox"
                  aria-expanded={recrutadorPopoverOpen}
                  aria-labelledby="recrutador-candidato-label"
                    ...
                    <CommandGroup>
                      {colaboradores.map((c) => (
                        <CommandItem
                          ...
                          value={String(c.nm_Profissional ?? c.cd_Profissional ?? c.codigoColaboradorInterno)}
                          onSelect={() => {
                            setSelectedCodigo(c.codigoColaboradorInterno);
                            setRecrutadorPopoverOpen(false);
                          }}
                        >
                          {c.nm_Profissional || c.cd_Profissional || c.codigoColaboradorInterno}
                        </CommandItem>
                      ))}
                    </CommandGroup>
                  </CommandList>
