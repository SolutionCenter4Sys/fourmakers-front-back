...
DROP TEMPORARY TABLE IF EXISTS tmp_result;
    ...
    SET v_sql = CONCAT('CREATE TEMPORARY TABLE tmp_result (
                                                            ...
                                                            EmpresaRelacionada VARCHAR(255),
...
SELECT
    ...
    tclo.nome_cliente AS __Cliente,
...
FROM
    tb_colaborador_apontamento ca
        ...
        LEFT JOIN tb_parametro_configuracao tpc ON tpc.valor_parametro = co.modelo_contratacao AND tpc.tb_org_id = co.tb_org_id AND tpc.codigo_parametro = 'OCULTAR_CONSULTAS_TIMESHEET_MODELO_CONTRATACAO'
WHERE
    ca.tb_org_id = org_id
