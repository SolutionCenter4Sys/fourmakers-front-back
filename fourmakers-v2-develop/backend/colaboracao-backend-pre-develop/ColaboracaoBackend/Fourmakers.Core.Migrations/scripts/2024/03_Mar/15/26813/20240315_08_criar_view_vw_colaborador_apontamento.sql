CREATE VIEW `vw_colaborador_apontamento` AS
    ...
    FROM
        ...
        JOIN `tb_projeto_gerente` `tpg` ON ((`tpo`.`cod_projeto` = `tpg`.`cod_projeto`)))
        JOIN `tb_colaborador_org` `tco_gerente` ON ((`tpg`.`cod_colaborador_gerente` = `tco_gerente`.`cod_colaborador_externo`)))
        JOIN `tb_colaborador` `tc_gerente` ON ((`tco_gerente`.`tb_colaborador_cpf` = `tc_gerente`.`cpf`)))
        JOIN `tb_projeto_org_atividade` `tpoa` ON ((`tpo`.`cod_projeto` = `tpoa`.`tb_projeto_org_cod_projeto`)))
        JOIN `tb_atividade` `ta` ON ((`tpoa`.`tb_atividade_id` = `ta`.`id`)))
