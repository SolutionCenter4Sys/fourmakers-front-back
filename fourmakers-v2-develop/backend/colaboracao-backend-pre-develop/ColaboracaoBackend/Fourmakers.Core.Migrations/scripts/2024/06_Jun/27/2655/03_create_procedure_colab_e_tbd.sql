...
BEGIN
    SELECT 
        ...
        -- tcpa.id AS colaborador_alocado_id, 
        ...
        CASE 
            WHEN tcpa.cod_tbd_alocado IS NOT NULL THEN tcpa_tbd.codigo_projeto
            ELSE tcpa.codigo_projeto
        END AS codigo_projeto,
        CASE 
            WHEN tcpa.cod_tbd_alocado IS NOT NULL THEN tpo_tbd.projeto
            ELSE tpo.projeto
        END AS projeto,
        tpo.cod_cliente,
        tpo.cliente,
    ...
    LEFT JOIN
        tb_tbd_alocado ttbd ON tcpa.cod_tbd_alocado = ttbd.cod_tbd_alocado
    LEFT JOIN 
        tb_colaborador_alocado tcpa_tbd ON ttbd.cod_tbd_alocado = tcpa_tbd.cod_tbd_alocado
    LEFT JOIN
        tb_projeto_org tpo_tbd ON tcpa_tbd.codigo_projeto = tpo_tbd.cod_projeto AND tpo_tbd.tb_org_id = tcpa.tb_org_id
    WHERE
        ...
        (p_pesquisa IS NULL OR
         ...
         (tcpa.cod_tbd_alocado IS NULL AND tcpa.codigo_projeto = p_pesquisa) OR
