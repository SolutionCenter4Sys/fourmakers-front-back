...
BEGIN
    SELECT 
        ...
        CASE 
            WHEN tca.cod_tbd_alocado IS NOT NULL THEN tca_tbd.codigo_projeto
            ELSE tca.codigo_projeto
        END AS codigo_projeto,
        CASE 
            WHEN tca.cod_tbd_alocado IS NOT NULL THEN tpo_tbd.projeto
            ELSE tpo.projeto
        END AS projeto,
        tpo.cod_cliente,
        tpo.cliente,
    ...
    LEFT JOIN
        tb_tbd_alocado ttbd ON tca.cod_tbd_alocado = ttbd.cod_tbd_alocado
    LEFT JOIN 
        tb_colaborador_alocado tca_tbd ON ttbd.cod_tbd_alocado = tca_tbd.cod_tbd_alocado
    LEFT JOIN
        tb_projeto_org tpo_tbd ON tca_tbd.codigo_projeto = tpo_tbd.cod_projeto AND tpo_tbd.tb_org_id = tca.tb_org_id
    WHERE
        ...
        (p_pesquisa IS NULL OR
         ...
         (tca.cod_tbd_alocado IS NULL AND tca.codigo_projeto = p_pesquisa) OR
