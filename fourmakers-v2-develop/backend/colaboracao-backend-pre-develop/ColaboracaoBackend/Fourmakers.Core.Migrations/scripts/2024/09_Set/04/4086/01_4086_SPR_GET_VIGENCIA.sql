...
BEGIN
    ...
    SELECT
        ...
        END AS projeto,
        -- Subconsulta para buscar o aprovador correto baseado na vigência e no status de aprovação
        (
            SELECT CASE WHEN resultado_select.cod_status_apontamento_periodo IN (5) THEN tc_aprovador.nome_completo ELSE NULL END AS aprovador
            FROM tb_colaborador_apontamento tca_aprovador
			JOIN tb_status_apontamento tsa_aprovador ON tca_aprovador.tb_status_apontamento_id = tsa.id
            JOIN tb_colaborador tc_aprovador ON tca_aprovador.codigo_interno_colaborador_alteracao = tc_aprovador.codigo_interno_colaborador
            JOIN tb_colaborador_org tco_aprovador ON tca_aprovador.codigo_interno_colaborador_alteracao = tco_aprovador.codigo_interno_colaborador
            WHERE tsa_aprovador.cod_status_apontamento = 5
			  AND tc_aprovador.ativo = 1
              AND tco_aprovador.ativo = 1
              AND tca_aprovador.codigo_interno_colaborador = resultado_select.codigo_interno_colaborador
              AND tca_aprovador.tb_vigencia_id = (SELECT id FROM tb_vigencia WHERE mes = p_mes AND ano = p_ano)
              AND tca_aprovador.codigo_interno_colaborador_alteracao IS NOT NULL
              AND tca_aprovador.tb_projeto_org_cod_projeto = tpo.cod_projeto
            LIMIT 1
        ) AS aprovador
    FROM
    (
    ...
    WHERE 
        ...
        AND (COALESCE(p_cod_colaborador_externo_aprovador, 0) = 0 OR resultado_select.codigo_interno_colaborador_alteracao = p_cod_colaborador_externo_aprovador);
