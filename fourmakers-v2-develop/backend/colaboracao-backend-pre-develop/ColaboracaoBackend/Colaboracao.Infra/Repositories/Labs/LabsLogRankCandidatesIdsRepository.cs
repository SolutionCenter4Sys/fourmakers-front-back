...
{
    /// <summary>
    /// Implementação do repositório de log do RankCandidatesIds (tb_labs_log_rank_candidates_ids).
    /// </summary>
    public class LabsLogRankCandidatesIdsRepository : ILabsLogRankCandidatesIdsRepository
    {
        private readonly IDBConnection _dapperConnection;

        public LabsLogRankCandidatesIdsRepository(IDBConnection dapperConnection)
        {
        ...
        public async Task<Guid> InserirAsync(int tbOrgId, string? idVaga, string? codigoInternoColaborador, string? objetoRequest, string? objetoResponse)
        {
            ...
            const string sql = @"INSERT INTO tb_labs_log_rank_candidates_ids
        ...
        {
            var connection = _dapperConnection.GetConnection();
            const string sql = "SELECT 1 FROM tb_labs_log_rank_candidates_ids WHERE id = @id LIMIT 1;";
