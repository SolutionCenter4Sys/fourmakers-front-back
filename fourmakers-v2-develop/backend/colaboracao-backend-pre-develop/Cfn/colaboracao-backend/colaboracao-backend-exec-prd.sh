#!/usr/bin/env sh
# ecs update-service --cluster ecs --service srv-colaboracao-backend-prd --enable-execute-command --force-new-deployment
set -e

CLUSTER="${CLUSTER:-ecs}"
SERVICE="${SERVICE:-srv-colaboracao-backend-prd}"
CMD="${*:-/bin/sh}"
