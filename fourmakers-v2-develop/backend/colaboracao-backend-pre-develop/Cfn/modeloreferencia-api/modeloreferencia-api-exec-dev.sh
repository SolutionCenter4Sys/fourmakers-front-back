#!/usr/bin/env sh
# ecs update-service --cluster ecs --service srv-modeloreferencia-api-dev --enable-execute-command --force-new-deployment
set -e

CLUSTER="${CLUSTER:-ecs}"
SERVICE="${SERVICE:-srv-modeloreferencia-api-dev}"
CMD="${*:-/bin/sh}"
