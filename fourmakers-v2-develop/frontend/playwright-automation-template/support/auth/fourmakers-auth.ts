...
export async function loginFourMakers(
  ...
  await page.waitForLoadState('domcontentloaded', { timeout: 60_000 }).catch(() => {
    // Algumas páginas do app podem atrasar o DOMContentLoaded no primeiro boot.
    // O token pode ser injetado após o commit da navegação.
  })
  await page.evaluate(
    ({ token, orgId }) => {
  ...
  try {
    ...
    // Em alguns cenários headed o reload pode ser abortado durante troca de rota.
    // Nesse caso, o fallback é navegar novamente para a mesma origem.
