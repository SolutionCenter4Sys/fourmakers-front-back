/**
 * Modal de drilldown "Gestores Impactados" do Dashboard Comercial.
 * Exibe dados agrupados por gestor conforme endpoint GestoresImpactadosDetalhe.
 */
...
import type { FiltrosComercial, GestoresImpactadosDetalhe } from '@shared/types/dashboardComercialTypes'
...
export interface DrilldownGestoresImpactadosModalProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  titulo: string
  comoCalculamos: string
  filtrosAtivos: FiltrosComercial
  dados: GestoresImpactadosDetalhe[] | null
  carregando: boolean
  erro: string | null
}

export function DrilldownGestoresImpactadosModal({
  open,
  onOpenChange,
  titulo,
  comoCalculamos,
  ...
  carregando,
  erro,
}: DrilldownGestoresImpactadosModalProps) {
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Listagem de gestores impactados e suas agendas no período.
          </DialogDescription>
        </DialogHeader>
          ...
          {carregando ? (
            <div className="flex flex-1 items-center justify-center rounded-lg border border-border bg-muted/30 p-12">
              <div className="flex flex-col items-center gap-3">
                ...
                <p className="text-sm text-muted-foreground">Carregando gestores impactados...</p>
              </div>
            </div>
          ) : erro ? (
            ...
            <div className="rounded-md border border-border p-8 text-center text-muted-foreground">
              Nenhum gestor impactado encontrado
            </div>
          ) : (
            <div className="space-y-6">
              {dados.map((gestor) => (
                <div key={gestor.codigoGestorExterno} className="rounded-lg border border-border bg-muted/20 p-4">
                  <div className="mb-3 flex items-baseline gap-2">
                    <span className="font-semibold text-foreground">{gestor.nomeGestor ?? '—'}</span>
                    <Badge variant="secondary" className="text-xs">
                      {(gestor.agendas?.length ?? 0)} agenda(s)
                    </Badge>
                  </div>
                  {!gestor.agendas?.length ? (
