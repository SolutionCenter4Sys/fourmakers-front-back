...
Qualquer uma dessas frases dispara `npm run demo` na raiz do repo (sobe Vite, roda Playwright, desliga Vite).
...
|---|---|
| `playwright-automation-template/support/auth/fourmakers-auth.ts` | **Login OTP QA Master** (credenciais fixas, endpoint real) |
| `playwright-automation-template/support/pages/ReembolsoPage.ts` | **Page Object** — locators e ações mapeados do DOM |
| `playwright-automation-template/fixtures/reembolso.json` | **Dados de teste** — objetivo, datas, valores, descrição |
| `DEMO/ui-elements/reembolso-ui.json` | Referência dos seletores reais capturados da UI |
| `DEMO/cenarios-bdd/REEMBOLSO-BDD-vN.md` | IDs e estrutura dos cenários BDD |
...
> **Ref. completa:** `DEMO/integracao-acesso-qa/`
...
cd fourmakers-v2-develop/playwright-automation-template
...
> `playwright-automation-template/` — projeto independente com `package.json` próprio
